...
{
    for (size_t i = 0; i < numInputs_; ++i) {
        INFO_LOG("numInputs_ is [%zu]", numInputs_);
        auto size = GetInputSize(i);
	...
	auto ret = aclnnAddcmulCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], inputTensor_[2],inputTensor_[3],outputTensor_[0],
    ...
    // 调用算子
    ret = aclnnAddcmulCustom(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
