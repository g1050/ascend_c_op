...
class KernelFastGeluGrad{
    public:
        ...
        __aicore__ inline void Init(GM_ADDR dy, GM_ADDR x,GM_ADDR z, uint32_t totalLength,uint32_t tileNum){
            ...
            pipe.InitBuffer(tmpAdd2H, this->tileLength*sizeof(float));
            pipe.InitBuffer(tmpExpXH, this->tileLength*sizeof(float));
    ...
    private:
        ...
        __aicore__ inline void Compute(int32_t progress){
            ...
            LocalTensor<float> add2H = tmpAdd2H.template Get<float>();
            LocalTensor<float> expXH = tmpExpXH.template Get<float>();
            ...
            Exp(expPnX,expPnX,this->tileLength);
            Cast(add2H,add2,RoundMode::CAST_NONE,this->tileLength);
            Cast(expXH,expX,RoundMode::CAST_NONE,this->tileLength);
            add2H = add2H + expXH;
            Cast(expXH,expPnX,RoundMode::CAST_NONE,this->tileLength);
            add2H = add2H + expXH;
            Cast(divUp,add2H,RoundMode::CAST_NONE,this->tileLength);

            Adds(divDown,expX,one,this->tileLength);
            Adds(divDown,expX,one,this->tileLength);
            Cast(add2H,divDown,RoundMode::CAST_NONE,this->tileLength);
            add2H = add2H * add2H;
            Cast(divDown,add2H,RoundMode::CAST_NONE,this->tileLength);
            z = divUp / divDown;
            z = z * dy;
...
template<typename dataType>
class KernelFastGeluGradH{ // For fp32, doesn't need cast.
    public:
        __aicore__ inline KernelFastGeluGradH(){}
        __aicore__ inline void Init(GM_ADDR dy, GM_ADDR x,GM_ADDR z, uint32_t totalLength,uint32_t tileNum){
            this->blockNum = GetBlockNum();
            ASSERT(this->blockNum != 0 && "block dim can not be zero");
            this->blockLength = totalLength / this->blockNum;
